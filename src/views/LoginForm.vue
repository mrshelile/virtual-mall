      ...
      <div class="xl:w-10/12">
        <div class="block bg-white shadow-lg rounded-lg">
          <div class="lg:flex lg:flex-wrap g-0">
              ...
              <div class="md:p-12 md:mx-6">
                ...
                <form class="w-full max-w-sm" @submit.prevent="handleSubmit">
                    <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:w-2/3">
                        <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-teal-500"   type="email" v-model="email" >
                        </div>
                    </div>
                    <div class="md:flex md:items-center ">
                        <div class="md:w-1/3">
                            ...
                            <div class="md:w-2/3">
                            <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-teal-500" v-model="password" id="password" type="password" placeholder="">
                        </div>
                    </div>
                    <div class="md:flex md:items-center">
                        <div class="md:w-1/3"></div>
                        <div class="md:w-2/3 text-red-600">
                        {{error}}
                        </div>
                    </div>
                    ...
                    <div class="md:flex md:items-center mb-6 mt-6">
                        ...
                        class="inline-block px-6 py-2 border-2 border-teal-600 text-teal-600 font-medium text-xs leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out"
                        >
                        Sign Up
                        </router-link>
                    </div>
            ...
            <div
              class="lg:w-6/12 flex items-center lg:rounded-r-lg rounded-b-lg lg:rounded-bl-none bg-gradient-to-r from-teal-500 via-gray-500 to-teal-700 p-6">
...
export default {
 ...
 data(){
     return{
         ...
         is_agent:localStorage.getItem("isAgentSite"),
     }
 },
 created(){
 ...
 methods:{
   async handleSubmit(){
    ...
    // data= JSON.stringify(data)
      var response =await axios.post('api/auth/local',data);
      
      // console.log(response.data.user.is_agent);
      if(response.status===200)
      { 
        // console.log(localStorage.getItem("token"));
        var filter ="filters[user]="+response.data.user.id
        var response1 =await axios.get('api/agents/?populate=*&'+filter);
        if(response1.status==200)
            if(response1.data.data.length!=0)
            localStorage.setItem("storeID",response1.data.data[0].attributes.store.data.id)
        localStorage.setItem("isAgentSite",response.data.user.is_agent)
